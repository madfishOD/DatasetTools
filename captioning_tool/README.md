...
`review.html` містить результати для перегляду, `captions/` — робочі TXT для редагування. Після затвердження image/TXT пари потрапляють у `exports/<snapshot>/dataset/data/`. Згенеровані тексти спочатку мають статус `unreviewed`. Опис, який досяг ліміту токенів без завершення, записується як помилка з raw text у JSON та не експортується. Реалізовані manifest v1, CLI resume, історія TXT, затвердження через `--approve-all` і snapshots експорту. Базовий GUI підтримує вибіркове затвердження та Stop/Resume; налаштування мети, вибір моделей і поради описані в [ADVICE_MODELS.md](ADVICE_MODELS.md). Команди й обмеження: [PROJECTS.md](PROJECTS.md).
