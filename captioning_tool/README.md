...
У JSON зразків записуються модель/revision, device/dtype, час captioning, розмір входу, кількість токенів і snapshot пам’яті. MPS snapshot — значення в момент вимірювання, не пікова пам’ять. CUDA-шлях і новий Compact на Windows потребують окремого GPU-прогону; Mac-регіони та SAM ще не перевірені.
