...
Базовий формат v1 і resume реалізовані через CLI: стабільні source/sample IDs, точний імпорт TXT, історія правок, fingerprints стадій, атомарні записи та блокування одночасного запису. Повторний експорт затверджених captions створює snapshot через тимчасовий каталог; завершені snapshots не перезаписуються, незавершені ігноруються. Без змін inference не запускається, валідний snapshot використовується повторно. Перевірено 31 тест і реальне відновлення MPS після SIGINT та перенесення проєкту без джерел. Windows resume ще потребує окремого smoke. Команди та межі v1 — [PROJECTS.md](PROJECTS.md).

Цільова схема вище ширша за v1: region IDs, crops, групи, мета навчання, поради й прив’язка затвердження до навчальних масок залишаються наступними етапами. Поточне затвердження стосується лише незмінного зображення та конкретного caption.
