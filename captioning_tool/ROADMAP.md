...
Додано вибір Compact/Quality і device/dtype та базовий dataset-only експорт. Базовий GUI captioning уже підтримує відкриття/імпорт, редагування, вибіркове затвердження, Stop/Resume та незалежний експорт. Додано вибір моделей із підтримуваного каталогу, мету навчання та базові поради для snapshot. Ще відсутні довільні нові архітектури моделей, crops, групування та експорт навчальних масок. Деталі: [ADVICE_MODELS.md](ADVICE_MODELS.md). `--resolution` задає конфігурацію навчання, але не готує зображення. `characters` і `person_N` обмежують поточний формат персонажами. `masked_training` явно вимкнений.
...
Стан після першого експерименту: окреме Python 3.13 arm64 середовище й Mac launcher створено, Qwen3-VL-2B на MPS float16 успішно обробила 12 фото Beans. Default prompts виправлено, device path спільний. Додатково завершено 40/40 captions на COCO128; regions/SAM smoke дав 5/6 успішних відповідей і 21 маску, одна відповідь обірвалася також із лімітом 2048. Повні заміри пам’яті, ілюстрації та стабільність регіонів ще потребують роботи; див. MAC_TESTING.md і COCO_TESTING.md. Подальші агентські перевірки лише технічні, без візуального порівняння входів і результатів.
...
Базова реалізація й відтворення тестів: [GUI.md](GUI.md). На Mac перевірено headless Qt з реальним MPS worker: Stop після поточної операції, Resume без повторення готового caption та нуль роботи на повторному запуску. Нижче лишається повний цільовий обсяг, не перелік уже готових функцій.
...
Базовий формат v1 і resume реалізовані через CLI: стабільні source/sample IDs, точний імпорт TXT, історія правок, fingerprints стадій, атомарні записи та блокування одночасного запису. Повторний експорт затверджених captions створює snapshot через тимчасовий каталог; завершені snapshots не перезаписуються, незавершені ігноруються. Без змін inference не запускається, валідний snapshot використовується повторно. Перевірено 38 тестів і реальне відновлення MPS після SIGINT та перенесення проєкту без джерел. Windows resume ще потребує окремого smoke. Команди та межі v1 — [PROJECTS.md](PROJECTS.md).

Цільова схема вище ширша за v1: region IDs, crops, групи й прив’язка затвердження до навчальних масок залишаються наступними етапами. Поточне затвердження стосується лише незмінного зображення та конкретного caption.
...
| 2. Формат проєкту та resume через CLI (база виконана на Mac) | Versioned manifest, стабільні IDs, імпорт TXT, атомарні записи, повторний експорт | Відновлення після аварій під час запису, між стадіями й посеред експорту; відсутність дублікатів; збереження початкових TXT і ручних правок; відкриття після перенесення проєкту |
| 3. GUI captioning та незалежний експорт (базовий GUI виконано; мета, базові поради й вибір моделей виконані; повний UX ще попереду) | Мета навчання, папки, presets, прогрес, перегляд/правки, Stop/Resume; image/TXT + manifest + TRAINING_ADVICE.txt; перший рецепт рекомендацій | Експорт без встановленого trainer; поради відповідають лише експортованим зразкам і змінюються з метою; невідомий контекст не породжує точних параметрів; числа порад і підтримуваного config узгоджені; усі три політики TXT працюють; аварія worker не закриває GUI; inference з кешем без мережі |
