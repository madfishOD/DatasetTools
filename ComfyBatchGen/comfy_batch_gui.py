...
# -*- coding: utf-8 -*-
"""
ComfyUI Batch GUI: queue each line as a separate job (with repeats).
- Reads editor JSON or API JSON. If editor JSON, converts to API format (best effort).
- If a TXT file is set: uses that list (one prompt per line). Else: reads multiline text from the chosen node.
- Posts each prompt to /prompt, replacing the target node's text input.
- "Repeats" controls how many times to queue each prompt line.

Notes:
- This uses only HTTP API (no custom nodes required). Your server must have the node classes your graph uses.
- For complex/custom nodes, prefer saving API format from ComfyUI UI when possible.
"""
...
def convert_editor_to_api(editor: dict) -> dict:
    """Best-effort converter from editor JSON to API JSON."""
    def widget_map(n):
...
class BatchGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Browse…", command=self._pick_json).grid(row=1, column=2, sticky="e")
        ...
        ttk.Button(top, text="…", width=3, command=self._pick_txt).grid(row=2, column=2, sticky="e")
        ...
        ttk.Spinbox(top, from_=1, to=10000, textvariable=self.repeats_var, width=8).grid(row=4, column=1, sticky="w", padx=(6,0))
    ...
    def _pick_json(self):
        p = filedialog.askopenfilename(title="Select Workflow JSON", filetypes=[("JSON files","*.json"),("All","*.*")])
        if p: self.json_var.set(p)

    def _pick_txt(self):
        p = filedialog.askopenfilename(title="Select Prompts TXT", filetypes=[("Text files","*.txt"),("All","*.*")])
        if p: self.txt_var.set(p)
    ...
    def _worker(self):
        ...
        try:
            ...
            graph = load_any_workflow(json_path)
            ...
            # Collect lines
            if txt_path:
                lines = get_lines_from_txt(txt_path)
            ...
            if not lines:
                raise RuntimeError("No prompts found after filtering (empty lines and lines starting with '#' are ignored).")
